...
struct ApplicationSettingsStorage
{
	...
	bool relay = false;
	String relay_topic; //temp_adc_pub
	String relay_topic_old;
	...
	void loadPeriph()
	{
		...
		if (existPeriph())
		{
			...
			rcswitch_topic = periph["rcswitch_topic"].asString();
			String tmp = periph["rcswitch_count"].asString();
			if (tmp.length() >= 1)
				rcswitch_count = atoi(tmp.c_str());

			if (rcswitch_count > 0) {
				JsonObject& rcdev = periph["rcswitch_dev"];
	...
	void savePeriph()
	{
		...
		periph["relay_topic"] = relay_topic;
