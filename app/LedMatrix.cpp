 ...
 */
void LedMatrix::init(uint8_t numDevices, int8_t sspin) {
	mySlaveSelectPin = sspin;
    pinMode(mySlaveSelectPin, OUTPUT);
    
    SPI.begin ();
    myNumberOfDevices = numDevices;
    //SPI.setDataMode(SPI_MODE0);
    //SPI.setClockDivider(SPI_CLOCK_DIV128);
    for(byte device = 0; device < myNumberOfDevices; device++) {
    //for(byte device = 0; device < numDevices; device++) {
...
uint8_t LedMatrix::getCharWidth() {
	return myCharWidth;
}
