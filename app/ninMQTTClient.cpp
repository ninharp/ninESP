...
void ninMqttClient::setKeepAlive(int seconds)
{
	keepAlive = seconds;
}

void ninMqttClient::setPingRepeatTime(int seconds)
{
...
{
	ninMqttClient* client = (ninMqttClient*)userInfo;
...
err_t ninMqttClient::onReceive(pbuf *buf)
{
	if (buf == NULL)
	{
...
void ninMqttClient::setHost(String host)
{
	server = host;
	mqtt_init(&broker);
}

void ninMqttClient::setPort(char* port)
{
	this->port = atoi(port);
	mqtt_init(&broker);
}

void ninMqttClient::setPort(String port)
{
...
void ninMqttClient::onReadyToSendData(TcpConnectionEvent sourceEvent)
{
